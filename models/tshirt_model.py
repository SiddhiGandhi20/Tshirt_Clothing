...
class TshirtModel:
    def __init__(self, db):
        self.collection = db.tshirts  # Reference to the 'tshirts' collection in MongoDB

    def create_tshirt(self, data):
        try:
            tshirt_data = {
                "name": data["name"],
            ...
            result = self.collection.insert_one(tshirt_data)
            return {"message": "T-shirt created successfully", "tshirt_id": str(result.inserted_id)}, 201
        except Exception as e:
            return {"message": "Error creating T-shirt", "error": str(e)}, 500
