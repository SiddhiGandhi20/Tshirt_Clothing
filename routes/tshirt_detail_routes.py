...
from models.tshirt_details_model import TshirtsDetailsModel  # Import tshirtDetailsModel
...
def create_tshirts_details_routes(db):
    ...
    def create_tshirts_details():
        try:
            ...
            if not tshirt:
                return jsonify({"message": "tshirt not found"}), 404
            ...
            tshirts_details_id = tshirts_details_model.create_item(name, price, image_url, tshirt_id)

            if tshirts_details_id:
                return jsonify({"id": tshirts_details_id, "name": name, "price": price, "image_url": image_url}), 201
    ...
    # Route: Get all tshirts_detailss
    @tshirts_details_bp.route("/tshirts_details", methods=["GET"])
    def get_all_tshirts_detailss():
        try:
            tshirts_details_model = TshirtsDetailsModel(db)
            tshirts_detailss = tshirts_details_model.get_all_items()
            return jsonify(tshirts_detailss), 200
        except Exception as e:
            return jsonify({"message": f"Error fetching tshirts_detailss: {str(e)}"}), 500

    # Route: Get tshirts_details by ID
    @tshirts_details_bp.route("/tshirts_details/<id>", methods=["GET"])
    def get_tshirts_details_by_id(id):
        try:
            tshirts_details_model = TshirtsDetailsModel(db)
            tshirts_details = tshirts_details_model.get_item_by_id(id)
            if tshirts_details:
                return jsonify(tshirts_details), 200
            return jsonify({"message": "tshirts_details not found"}), 404
        except Exception as e:
            return jsonify({"message": f"Error fetching tshirts_details: {str(e)}"}), 500

    # Route: Update a tshirts_details
    @tshirts_details_bp.route("/tshirts_details/<id>", methods=["PUT"])
    def update_tshirts_details(id):
        try:
            tshirts_details_model = TshirtsDetailsModel(db)
            tshirts_details = tshirts_details_model.get_item_by_id(id)
            ...
            # Get tshirt_id from the form data
            ...
            # Update in database
            success = tshirts_details_model.update_item(id, updated_data)
            if success:
                updated_tshirts_details = tshirts_details_model.get_item_by_id(id)
    ...
    @tshirts_details_bp.route("/tshirts_details/<id>", methods=["DELETE"])
    def delete_tshirts_details(id):
        try:
            tshirts_details_model = TshirtsDetailsModel(db)
            tshirts_details = tshirts_details_model.get_item_by_id(id)
            if not tshirts_details:
            ...
            success = tshirts_details_model.delete_item(id)
            if success:
                return jsonify({"message": "tshirts_details deleted successfully"}), 200
